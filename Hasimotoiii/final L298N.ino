...
const int ledR = 5; //RBG LEDのRピン RED GREEN BLUEです。PWM
...
//受信関数
void rx() {
  pulseNum1 = pulseIn(ch1, HIGH); //Left Y
...
void loop() {
  ...
  if (pulseNum1 != 0) {
    ...
    int speedVal4 = map(pulseNum4, 2000, 1000, -255, 255); //ここも整数でおっけー
    ...
    if (state == 1) {
      if (pulseNum7 <= 1400) {
        analogWrite(ledR, R) ;             // 　９番ピンから赤LEDの出力
        analogWrite(ledG, G) ;              // １１番ピンから緑LEDの出力
        analogWrite(ledB, B) ;              // １０番ピンから青LEDの出力

      }

      else {
        C1 = random(0, 255); // 10から19の乱数を生成
        C2 = random(0, 255); // 10から19の乱数を生成
        C3 = random(0, 255); // 10から19の乱数を生成

        analogWrite(ledR, C1) ;             // 　９番ピンから赤LEDの出力
        analogWrite(ledG, C2) ;              // １１番ピンから緑LEDの出力
        analogWrite(ledB, C3) ;              // １０番ピンから青LEDの出力
      }
    }
    ...
    Serial.print("TX CONNECTED");
    Serial.print("  ch1: ");
    Serial.print(pulseNum1);
    Serial.print("  ch2: ");
    Serial.print(pulseNum2);
    Serial.print("  ch4: ");
    Serial.print(pulseNum4);
    Serial.print("  aux1: ");
    Serial.print(pulseNum5);
    Serial.print("  aux2: ");
    Serial.print(pulseNum6);
    Serial.print("  aux3: ");
    Serial.print(pulseNum7);
